...
- **Target hardware is the N64 with the Expansion Pak (8 MB RDRAM).** `rom_main.cpp` calls `assert_memory_expanded()` at boot, so the ROM fails early with a clear error screen if the pak is absent. The whole-map interconnected renderer's streaming/memory budget assumes the full 8 MB heap. The authoritative device figure is the `[memory]` mallinfo arena: stationary `total` ≈ **2.16 MB**, `used` ≈ **2.15 MB** (ratio ≈ 0.991); walking `total` ≈ **2.29 MB**, `used` ≈ **2.23 MB** (ratio ≈ 0.975 — the ring holds more resident cells while streaming). Both are stable across every capture in `.plans/n64-optimization/`. The older "≈ 5.48 MB arena / used ≈ 5.39 MB" figure is stale — it does not match any device telemetry. The 4 MB base-RDRAM target was dropped.
- **Build + render state (n64-optimization close-out):** per-TU compiler flags — 6 hot TUs (`tile_streamer.o`, `textured_room_renderer.o`, `lvl_room_renderer.o`, `open_world_renderer.o`, `camera_controller.o`, `player_motor.o`) build at `-O2` via target-specific `CXXFLAGS += -O2` Makefile overrides while the global build stays `-Os` (verify with `make -nB all`). Near-pass culling gate `kEnableNearCulling` (`tile_streamer.hpp`, default **OFF**) gates the `CellAabbInNearCone` per-resident-cell test in `TileStreamer::UpdateCamera` (center cell always drawn). Dead code removed (Inc 6): `distant_world_renderer.cpp`, `gameplay/arena.*`, `tests/runtime_smoke.cpp` are deleted; the `.dlod` files + baker (`tools/writers/dlod_writer.py`, `tools/ogworld/distant_lod.py`) remain on disk for a future distant-horizon feature and are not loaded at runtime.
